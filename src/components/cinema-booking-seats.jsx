...
const CinemaBookingSeats = ({
  ...
  seatType = {
    ...
    premium: { name: "Permiun", price: 250, rows: [3, 4, 5] },
    vip: { name: "VIP", price: 350, rows: [6, 7] },
  },
  bookedSeats = ["C2", "C4"],
  currency = "$",
  onBookingComplete = () => {},
  title = "Bollywood Multiplex",
  ...
  const getSeatType = (row) => {
    const seatTypeEnteries = Object.entries(seatType);
    console.log("seatType: ", seatType);

    for (let i = 0; i < seatTypeEnteries.length; i++) {
      const [type, config] = seatTypeEnteries[i];
  ...
  const getSeatClassName = (seat) => {
    const baseClass =
      "w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 m-1 rounded-t-lg border-2 cursor-pointer transition-all duration-200 flex items-center justify-center text-xs s:text-sm font-bold bg-blue-100 border-blue-300 text-blue-800";
  ...
  const handleBooking = () => {
    ...
    onBookingComplete({
      ...
      seatsIds: selectedSeats.map((seat) => seat.id),
    });
  ...
  return (
    <div className="w-full min-h-screen bg-gray-50 p-4">
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-lg p-6">
        ...
        {/* screen */}
        <div className="mb-8">
          <div className="w-full h-4 bg-linear-to-r from-gray-300 via-gray-400 to-gray-300 rounded-2xl mb-2 shadow-inner" />
...
export default CinemaBookingSeats;
